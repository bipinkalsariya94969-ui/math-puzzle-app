...
window.state = {
  user: null, // { email, name, score, coins, streak, lastLogin }
  currentScore: 0,
  currentCoins: 0,
  currentGame: null,
  timer: null,
...
window.updateUI = function() {
  ...
  // Streak & Goal (Snapchat/Duolingo style)
  const streakVal = document.getElementById('user-streak-val');
  if (streakVal) streakVal.innerText = `🔥 ${window.state.user.streak}`;
  
  const dailyGoalPercent = Math.min(100, (window.state.user.gamesPlayedToday || 0) * 20); // 5 games = 100%
